...
For example, for the user command "stabilize the video shaky_video.mp4", your response should be:
```json
{json.dumps(example_params, indent=2)}
```

Additional examples:
- "Can you stabilize clip.mov?" -> {{"videoPath": "clip.mov"}}
- "I want to stabilize the footage in video_001.avi" -> {{"videoPath": "video_001.avi"}}
- "stabilize video" -> {{"videoPath": "NULL"}} (if no video is mentioned or selected)
"""
